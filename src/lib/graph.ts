...
export async function assignLicense({ licenseId, userId, accessToken }: AssignLicenseProps) {
  await axios({
    method: 'post',
...
export async function removeLicense({ licenseId, userId, accessToken }: RemoveLicenseProps) {
  await axios({
    method: 'post',
    url: `https://graph.microsoft.com/v1.0/users/${userId}/assignLicense`,
    headers: {
...
export async function resetUserPassword({ userId, password, accessToken }: ResetUserPasswordProps) {
